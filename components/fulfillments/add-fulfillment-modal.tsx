...
import { AddFulfillmentForm, FulfillmentFormData } from "@/components/fulfillments/add-fulfillment-form";
import { Fulfillment } from "@/lib/types/fulfillments";

interface AddFulfillmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (fulfillmentData: FulfillmentFormData) => Promise<void>;
  editingFulfillment?: Fulfillment | null;
}

export function AddFulfillmentModal({ isOpen, onClose, onSubmit, editingFulfillment }: AddFulfillmentModalProps) {
  ...
  const handleSubmit = async (fulfillmentData: FulfillmentFormData) => {
    try {
      setIsLoading(true);
      await onSubmit(fulfillmentData);
      onClose();
    } catch (error) {
      ...
      >
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">
            {editingFulfillment ? 'Edit Fulfillment' : 'Add New Fulfillment'}
          </h2>
          <Button
            variant="ghost"
            size="sm"
